...
Attach metadata that updates on each poll by setting the `MetadataFile` and `MetadataVars` properties. The metadata is loaded from a separate .mat file and attached to the specified line.
...
For dashboards, set the properties on the grid figure:
...
fig.startLive('sensors.mat', @updateDashboard);
```
...
- Use `DeferDraw = true` on a `FastSense` instance to skip `drawnow` during batch render for better performance.
- The `updateData` method accepts optional `'Metadata'` and `'SkipViewMode'` arguments for advanced control.

For real‑time event monitoring with thresholds and live pipelines, see [[API Reference: Event Detection]].
...
- [[API Reference: Event Detection]] — Event‑based monitoring
- [[Examples]] — `example_dashboard_live.m`, `example_live_pipeline.m`
- [[API Reference: Themes]] — styling live plots
- [[API Reference: FastSenseToolbar]] — toolbar integration
