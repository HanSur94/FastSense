...
|---------|--------|-------------|
| `example_basic` | 10M | Noisy sine wave with upper/lower thresholds and warning levels. Shows basic FastPlot workflow: addLine, addThreshold, render |
...
| Example | Description |
|---------|-------------|
| `example_dashboard` | 2x2 FastPlotFigure with bands, shading, fills, and markers. Shows setTileSpan, tileTitle |
| `example_dashboard_9tile` | 3x3 grid with 9 different signal types (15M+ total). Shows large grid layouts |
| `example_dock` | FastPlotDock with 5 tabbed dashboards, datetime axes, metadata. Full dock workflow |
...
| Example | Description |
|---------|-------------|
| `example_stress_test` | 5-tab FastPlotDock with 26 sensors across 60M total points. Tests rendering performance at scale |

## Dashboard Engine

| Example | Description |
|---------|-------------|
| `example_dashboard_engine` | DashboardEngine with sensor-bound FastPlotWidgets, dynamic thresholds, JSON save/load |
| `example_dashboard_all_widgets` | Every widget type in a single dashboard: FastPlot, Number, Status, Gauge, Table, RawAxes, Timeline, Text |
| `example_dashboard_live` | DashboardEngine in live mode with periodic data updates |
| `example_mixed_tiles` | Dashboard with mixed FastPlotFigure tiles and DashboardEngine widgets |

## Data Storage

| Example | Description |
|---------|-------------|
| `example_disk_storage` | FastPlotDataStore with SQLite-backed chunked storage for 100M+ datasets |
| `example_dock_disk` | FastPlotDock with disk-backed DataStore across multiple tabs |
...
| `example_dock_many_tabs` | FastPlotDock with many tabs and lazy rendering |
