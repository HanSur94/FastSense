...
│   │   ├── Tag.m                     # Abstract base class for domain model
│   │   ├── SensorTag.m               # Concrete sensor time-series
│   │   ├── StateTag.m                # Discrete state signals with ZOH
│   │   ├── MonitorTag.m              # Binary 0/1 derived signals
│   │   ├── CompositeTag.m            # Aggregate MonitorTag children
│   │   ├── TagRegistry.m             # Singleton tag catalog
│   │   ├── BatchTagPipeline.m        # Batch raw-data processing
│   │   ├── LiveTagPipeline.m         # Live raw-data processing
...
│   │   ├── EventViewer.m             # Interactive Gantt chart viewer
│   │   ├── LiveEventPipeline.m       # Timer-driven live detection
│   │   ├── NotificationService.m     # Email notifications with snapshots
│   │   ├── EventStore.m              # Thread-safe .mat file persistence
│   │   ├── EventBinding.m            # Many-to-many Event-Tag registry
│   │   ├── IncrementalEventDetector.m# State-carrying incremental detection
...
│   │   ├── DashboardSerializer.m     # JSON save/load and script export
│   │   ├── DashboardTheme.m          # Theme system with dashboard extensions
...
│   │   ├── StatusWidget.m            # Colored status indicator
│   │   ├── TextWidget.m              # Static text/headers
│   │   ├── TableWidget.m             # Data table display
│   │   ├── RawAxesWidget.m           # User plot function wrapper
│   │   ├── EventTimelineWidget.m     # Event bars on timeline
...
│   │   ├── ScatterWidget.m           # Scatter plot display
│   │   ├── HeatmapWidget.m           # Heatmap/matrix display
│   │   ├── HistogramWidget.m         # Histogram display
│   │   ├── ImageWidget.m             # Image display
│   │   ├── DividerWidget.m           # Horizontal separator line
│   │   ├── IconCardWidget.m          # Icon + value cards
│   │   ├── ChipBarWidget.m           # Row of status chips
│   │   ├── SparklineCardWidget.m     # KPI with mini chart
│   │   ├── TimeRangeSelector.m       # Time range scrubber
│   │   ├── DashboardPage.m           # Multi-page container
│   │   ├── DashboardProgress.m       # Render progress indication
│   │   ├── DetachedMirror.m          # Standalone widget windows
│   │   ├── DashboardConfigDialog.m   # Configuration editor
...
The new Tag system provides a unified interface for all data sources:

```
Tag (abstract base)
├── SensorTag     — Time-series sensor data
├── StateTag      — Discrete state signals (ZOH)
├── MonitorTag    — Derived 0/1 binary signals
└── CompositeTag  — Aggregated multi-tag signals
```

Key properties:
- **TagRegistry**: Singleton catalog with globally unique keys
- **Observer pattern**: Tags notify listeners on data updates
- **Lazy evaluation**: MonitorTag/CompositeTag compute on first access
- **Serialization**: Round-trip to/from JSON via two-phase loading
...
### MonitorTag Processing

MonitorTag provides lazy-by-default, cache-aware binary signal derivation:

1. **Condition evaluation**: User-supplied `ConditionFn` applied to parent Tag's (X,Y)
2. **Hysteresis**: Optional `AlarmOffConditionFn` for different on/off conditions
3. **Debouncing**: `MinDuration` filters short-duration violations
4. **Event emission**: Optional `EventStore` + callbacks for violation runs
5. **Persistence**: Opt-in disk caching via `FastSenseDataStore`
...
2. New data is passed to parent Tag via `updateData()`
3. MonitorTag processes via `appendData()` with state preservation
...
**EventBinding** provides a many-to-many registry linking Events to Tags:
- Forward index: `eventId -> {tagKeys}`
- Reverse index: `tagKey -> {eventIds}`
- Used by convenience methods on Event/Tag/EventStore
...
├── DashboardPage         — Multi-page support
├── TimeRangeSelector     — Global time scrubber with preview envelope
└── Widgets (DashboardWidget subclasses)
    ...
    ├── IconCardWidget         — Compact icon + value cards
    ├── ChipBarWidget          — Horizontal status chip strip
...
2. Each widget's `refresh()` is called (sensor-bound widgets re-read current value)
...
The Tag pipeline system processes raw delimited text files into per-tag .mat files:

### Batch Processing
- `BatchTagPipeline`: Synchronous processing of all tags at once
- File-level deduplication: each raw file is parsed once regardless of how many tags reference it
- Per-tag error isolation: one tag's failure doesn't abort the batch

### Live Processing
- `LiveTagPipeline`: Timer-driven incremental processing
- File modification time tracking with per-tag last-index state
- Append-only: new rows are appended to existing .mat files

### Common Processing
Both pipelines share:
- `readRawDelimited_()`: Auto-detects delimiter (comma, semicolon, tab, space)
- `selectTimeAndValue_()`: Extracts time + value columns by name or index
- `writeTagMat_()`: Saves in SensorTag.load() compatible format
