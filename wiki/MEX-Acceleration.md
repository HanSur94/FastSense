...
The build script auto‑detects your architecture and compiles all MEX functions with appropriate SIMD optimizations. On Octave it uses a platform‑tagged subdirectory (`private/octave-<platform>/`) to avoid extension clashes; on MATLAB the binaries are placed directly in `private/`.
...
- Converts threshold arrays into a compact step representation  
- Used when thresholds are defined as (X, Y) pairs  
...
**delimited_parse_mex** — Accelerated parsing of delimited sensor data  
- **Used by**: [[Sensors|API Reference: Sensors]] Tag pipeline  

### Concurrency Support

**Concurrency MEX** — Parallel execution primitives for sensor pipelines  
- Compiled automatically during `build_mex` if the `libs/Concurrency` source is present  
- Best‑effort: failure does not abort the main build  
...
6. **Copies shared files** — distributes violation/threshold MEX binaries to the SensorThreshold library
7. **Builds auxiliary MEX** — compiles SensorThreshold parsing kernels and Concurrency library (when present)
