...
| **SensorThreshold** | Tag-based sensor and threshold data model with monitoring rules, state channels, batch/live data pipelines, and TagRegistry catalog |
...
```matlab
% Advanced thresholds with violation markers and bands
fp = FastSense('Theme', 'industrial');
x = 1:1e5;
y = sin(x/1000) + 0.1*randn(1,1e5);
fp.addLine(x, y, 'DisplayName', 'Sensor Signal');
fp.addThreshold(0.5, 'Direction', 'upper', 'ShowViolations', true, 'Label', 'Upper');
fp.addThreshold(-0.5, 'Direction', 'lower', 'ShowViolations', true, 'Label', 'Lower');
fp.addBand(-0.5, 0.5, 'FaceAlpha', 0.1);
fp.render();
```
...
- [[Dashboard Engine Guide]] — widget-based dashboards with edit mode
