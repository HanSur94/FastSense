...
violationEvents = [ ...
    Event(9000,  9600,  'T-401', 'Hi Warn',  78, 'upper'), ...
    Event(18000, 19200, 'T-401', 'Hi Alarm', 85, 'upper'), ...
    Event(36000, 36900, 'P-201', 'Hi Warn',  65, 'upper'), ...
    Event(54000, 55800, 'T-401', 'Hi Warn',  78, 'upper')];

% --- EventStore with appended events ---
storePath = fullfile(tempdir, 'example_timeline_store.mat');
store = EventStore(storePath);
store.append(Event(10800, 12600, 'T-401', 'Hi Warn',  78, 'upper'));
store.append(Event(43200, 45000, 'P-201', 'Hi Alarm', 70, 'upper'));
store.append(Event(61200, 63000, 'T-401', 'Hi Alarm', 85, 'upper'));
