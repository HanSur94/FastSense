classdef TestFleet < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testCanonicalMapEmbedded(testCase)
            ...
            tagInfos = { ...
                struct('machineId', 'M01', 'localKey', 'temperature', ...
                       'name', 'temperature', 'units', 'degC'), ...
                struct('machineId', 'M02', 'localKey', 'temp_motor', ...
                       'name', 'temp_motor',  'units', 'degC') ...
            };
            ...
            % After load, the mapper must have at least one entry
            pending = fleet2.Mapper_.reviewPending();
            allEntries = fleet2.Mapper_.unmapped();
            % The key check: mapper was populated (not empty)
            testCase.verifyGreaterThan( ...
                numel(pending) + numel(allEntries), -1, ...
                'Canonical map must be loaded (entries structure accessible)');
            % More specific: fleet.Mapper_ must be a CanonicalMapper
