...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - libs/Fleet/Fleet.m lines 108-130 (machineCount, machineIds), CanonicalMapper keys via `keys(fleet.Mapper_.Entries_)`
  </read_first>
  <behavior>
    ...
    - The quick-fill dropdown Items equal the CanonicalMapper logical ids (keys of Mapper_.Entries_); with no mappings the dropdown is empty and the scroll panel shows 'No machines in fleet' only when machineCount==0.
  ...
  <verify>
    <automated>cd /Users/hannessuhr/PARA/10_Projects/FastPlot/.claude/worktrees/friendly-leakey-0bc166 && grep -q 'classdef CompareBuilderDialog' libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q "CompareBuilderDialog:invalidApp" libs/FastSenseCompanion/CompareBuilderDialog.m && grep -qv "WindowStyle" libs/FastSenseCompanion/CompareBuilderDialog.m && echo PASS</automated>
  </verify>
  <acceptance_criteria>
    - `mcp__matlab__check_matlab_code` clean on CompareBuilderDialog.m.
    - Smoke (evaluate): build a 3-machine Fleet + FastSenseCompanion('Fleet', fleet), construct CompareBuilderDialog(app), verify hFig_ valid + Name 'Compare Machines' + isvalid; then close cleanly (no leaked figure/timer).
    - Non-FastSenseCompanion arg throws CompareBuilderDialog:invalidApp.
...
<task type="auto" tdd="true">
  ...
  <files>libs/FastSenseCompanion/CompareBuilderDialog.m</files>
  <read_first>
    - .planning/phases/1045-cross-machine-comparison-view/1045-UI-SPEC.md "Machine Row Grid", "Row Column Specifications" (cols 1-6 exact widths/widgets), "Interaction Contract" → "Row State Machine" (state table + transition rules), "Per-Row Status Badge Copy" (exact glyph+text+FontColor per state)
  ...
  <verify>
    <automated>cd /Users/hannessuhr/PARA/10_Projects/FastPlot/.claude/worktrees/friendly-leakey-0bc166 && grep -q 'function rebuildRows_' libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q 'function resolveAllRows_' libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q 'buildCompareResolution_' libs/FastSenseCompanion/CompareBuilderDialog.m && echo PASS</automated>
  </verify>
  <acceptance_criteria>
    - `mcp__matlab__check_matlab_code` clean.
    - Smoke (evaluate): construct dialog on a 3-machine fleet with a CanonicalMapper suggested mapping where one machine is HIGH, one LOW+AUTO, one unmapped; set `hSensorDD_.Value` + invoke onSensorSelected_; assert RowStates_ states are {'auto','confirm_needed','none'} in insertion order and the count label / Open enable reflect includedCount.
    - LOW+AUTO row checkbox unchecked by default (invariant #4 at the UI level).
...
<task type="auto" tdd="true">
  ...
  <files>libs/FastSenseCompanion/CompareBuilderDialog.m</files>
  <read_first>
    - .planning/phases/1045-cross-machine-comparison-view/1045-UI-SPEC.md "'Open Comparison' Button Behavior" (steps 1-9), "Consolidated Open-Time Alert Copy" (exact unit-mismatch + skipped strings), "Legend Label Format"
  ...
  <verify>
    <automated>cd /Users/hannessuhr/PARA/10_Projects/FastPlot/.claude/worktrees/friendly-leakey-0bc166 && grep -q 'function onOpenComparison_' libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q "openAdHocPlot(" libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q 'ResolvedTags_' libs/FastSenseCompanion/CompareBuilderDialog.m && grep -q 'trackOpenedFigure_' libs/FastSenseCompanion/CompareBuilderDialog.m && echo PASS</automated>
  </verify>
  <acceptance_criteria>
    - `mcp__matlab__check_matlab_code` clean.
    - Smoke (evaluate): 2-machine fleet with mock SensorTags; select shared sensor; invoke onOpenComparison_; assert numel(ResolvedTags_)==2, an overlay figure exists and is in App_.OpenedFigures_, and SeriesLabels match '[Name]: [sensor]'. Close the spawned figure cleanly afterward.
    - Skipped machine produces a consolidated alert and the comparison still opens with the rest (CMP-03).
    - No CanonicalMapper method is referenced after the cache is populated (invariant #5 at the source level).
  </acceptance_criteria>
