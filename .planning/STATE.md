...
last_activity: 2026-05-13 -- Quick task 260513-q7w: Debounced post-resize refresh stops widgets going white during drag-resize — TWO parallel timers: 300 ms cheap two-pass refresh + 1.2 s unconditional rerenderWidgets() backstop (bulletproof "user has stopped resizing for a while" final redraw). Both restart on every resize event so neither fires during continuous drag. Re-entrancy guard self-reschedules the backstop if a render is in flight.
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
...
| 260513-q7w | Debounced post-resize refresh stops widgets going white during drag-resize — TWO parallel timers on every figure resize event: (1) 300 ms `ResizeDebounceTimer` runs a cheap two-pass sweep — Pass 1 calls `update()`/`refresh()` on every active-page widget, Pass 2 detects any FastSenseWidget whose line lost XData and escalates to per-widget refresh() then rerenderWidgets() if still white; (2) 1.2 s `ResizeFinalRedrawTimer` unconditionally calls `rerenderWidgets()` as a bulletproof backstop equivalent to the user pressing Reset, catching failure modes the cheap pass misses (destroyed line handles, degenerate axes after long holds at tiny window sizes, etc.). Both timers restart on every resize event so neither fires during continuous drag. Re-entrancy guard on the final redraw checks `Progress_` and self-reschedules if a render is in flight (initial figure render fires SizeChangedFcn). All teardown wired in delete(). test_dashboard_range_selector_integration 2/2, test_dashboard_time_sync_all_pages 5/5; synthetic white-widget test on live demo confirms recovery (XData 941→0→941 after forced wipe); both timers verified scheduling + firing cleanly | 2026-05-13 | 577bf95, 99c8808, 4eda604 | — | [260513-q7w-during-dashboard-figure-resize-fastsense](./quick/260513-q7w-during-dashboard-figure-resize-fastsense/) |
