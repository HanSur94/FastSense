...
last_activity: 2026-05-13 -- Quick task 260513-ovt: Preserve widget X and Y views across Live ticks + Follow toggle reaches every page — flipped FastSenseWidget.LiveViewMode default 'reset'→'preserve', made FastSenseToolbar.syncFollowState public (fixes auto-disengage button-state sync), made DashboardEngine.{allPageWidgets,activePageWidgets} public + onFollowToggle uses allPageWidgets() (fixes silent Follow no-op on multi-page dashboards), removed autoScaleY_(y) from FastSenseWidget.refresh/update + broadcastTimeRange from DashboardEngine.onLiveTick. Live mode now strictly appends data; Follow now actually toggles every widget across pages with width-preserving slide and 2% right-edge gap.
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
