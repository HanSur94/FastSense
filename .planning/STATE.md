...
last_activity: 2026-05-13 -- Quick task 260513-q7w: Debounced post-resize refresh stops widgets going white on figure resize — new DashboardEngine.ResizeDebounceTimer + scheduleResizeRefresh_() + refreshActivePageWidgetsAfterResize_() chain. After 300 ms of resize quiescence, all active-page widgets get an update()/refresh() call so any FastSenseWidget whose line data was wiped by a resize-race recovers without the user pressing Reset.
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
...
Last activity: 2026-05-13 - Completed quick task 260513-q7w: Debounced post-resize refresh. Symptom: FastSenseWidget panels going white during drag-resize and staying white until the user pressed Reset (rerenderWidgets). Root-cause hypothesis: the SizeChangedFcn cascade — onResize → repositionPanels → cell SizeChangedFcn → reflowChrome_ (Units flips) — interleaves with FastSense.getAxesPixelWidth's transient axes-Units flip and any pending updateLines call; under the wrong race lineVisibleData returns empty arrays leaving XData empty. With Live OFF nothing ever calls updateData again. Fix in DashboardEngine.m: new ResizeDebounceTimer + scheduleResizeRefresh_ + refreshActivePageWidgetsAfterResize_ (mirrors SliderDebounceTimer pattern), called at the end of onResize and torn down in delete(). After 300 ms quiescence every active-page widget gets a cheap update()/refresh() re-push — line data restored, no rerenderWidgets blink. Commit 577bf95. Tests: 2/2 + 5/5 PASS, live-engine probe confirms timer schedules with Tag='DashboardEngineResizeDebounce' and fires cleanly after 0.5 s.
...
| 260513-q7w | Debounced post-resize refresh stops widgets going white during drag-resize — added `DashboardEngine.ResizeDebounceTimer` + `scheduleResizeRefresh_()` + `refreshActivePageWidgetsAfterResize_()` (mirrors SliderDebounceTimer). After 300 ms of resize quiescence, every active-page widget gets a cheap update()/refresh() re-push (no rerenderWidgets sledgehammer); any FastSenseWidget whose line lost its XData to a resize-race recovers without the user pressing Reset. test_dashboard_range_selector_integration 2/2 PASS, test_dashboard_time_sync_all_pages PASS; live-engine probe confirms timer schedules with Tag='DashboardEngineResizeDebounce' and fires cleanly after 0.5 s | 2026-05-13 | 577bf95 | — | [260513-q7w-during-dashboard-figure-resize-fastsense](./quick/260513-q7w-during-dashboard-figure-resize-fastsense/) |
