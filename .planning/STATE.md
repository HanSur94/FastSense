...
last_activity: 2026-05-13 -- Quick task 260513-q7w: Debounced post-resize refresh stops widgets going white during drag-resize + proper switchPage serialization — switchPage now (a) cancels resize timers AND (b) waits up to 3 s for any in-flight rerenderWidgets to complete before mutating state, preventing the tab-button uicontrol callback from interrupting rerender mid-loop (which left old-page widgets visible and new-page widgets white).
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
...
| 260513-q7w | Debounced post-resize refresh stops widgets going white during drag-resize — TWO parallel timers on every figure resize event: (1) 300 ms `ResizeDebounceTimer` runs a cheap two-pass sweep — Pass 1 calls `update()`/`refresh()` on every active-page widget, Pass 2 detects any FastSenseWidget whose line lost XData and escalates to per-widget refresh() then rerenderWidgets() if still white; (2) 1.2 s `ResizeFinalRedrawTimer` unconditionally calls `rerenderWidgets()` as a bulletproof backstop. Both timers restart on every resize event so neither fires during continuous drag. Critical interaction fixes for switchPage races: (a) switchPage cancels both timers via `cancelResizeTimers_()`; (b) switchPage waits up to 3 s for any in-flight rerenderWidgets to complete before mutating state — rerenderWidgets uses internal drawnow that lets tab-button uicontrol callbacks interrupt it, and running switchPage mid-loop leaves old-page widgets visible AND new-page widgets white; (c) new `IsRerendering_` flag (set via onCleanup) makes `onResize` early-return during rerender cascades — panel teardown SizeChangedFcn events were scheduling new timers; (d) re-entrancy guard now aborts instead of self-rescheduling. All teardown wired in delete(). test_dashboard_range_selector_integration 2/2, test_dashboard_time_sync_all_pages 5/5; live demo reproduction confirms zero white widgets after resize→tab-switch (page 1 visible=0, page 2 visible=5) | 2026-05-13 | 577bf95, 99c8808, 4eda604, bc305dc, 54d5aa0 | — | [260513-q7w-during-dashboard-figure-resize-fastsense](./quick/260513-q7w-during-dashboard-figure-resize-fastsense/) |
