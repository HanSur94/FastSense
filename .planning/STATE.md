...
- [Phase 1012-migrate-examples-to-tag-api]: Plan 01: test_examples_smoke.m Octave-only (Option A); MATLAB CI already covers examples via matlab-examples job
- [Phase 1012-migrate-examples-to-tag-api]: Plan 01: Optional 'folder', <name> NV-pair on test_examples_smoke is the API every Wave 2 plan's <verify> block uses for per-folder green gating
...
- Phase 1012 added (2026-04-17): Migrate examples to Tag API — replace remaining legacy `Sensor(...)`, `addThresholdRule(...)`, `SensorRegistry` calls in `examples/` with `SensorTag` / `StateTag` / `MonitorTag` / `CompositeTag` / `TagRegistry`; fix half-migrated stubs (e.g. examples/02-sensors/example_sensor_threshold.m has orphan threshold comments with no code); follow-on to Phase 1009 consumer-migration.
