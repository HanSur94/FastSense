...
last_activity: 2026-05-13 -- Quick task 260513-ovt: Preserve widget X and Y views across Live ticks — flipped FastSenseWidget.LiveViewMode default 'reset'→'preserve', made FastSenseToolbar.syncFollowState public (fixes auto-disengage button-state sync), removed autoScaleY_(y) from FastSenseWidget.refresh/update + broadcastTimeRange from DashboardEngine.onLiveTick. Live mode now strictly appends data; axis limits change only on explicit user action.
progress:
  total_phases: 6
  completed_phases: 2
  total_plans: 13
...
Last activity: 2026-05-13 - Completed quick task 260513-ovt: Preserve widget X and Y views across Live ticks. Three commits: (1) 498a5f3 added a LiveViewMode='follow' guard inside FastSenseWidget.autoScaleY_; (2) ca5be95 removed autoScaleY_(y) calls from refresh/update entirely (initial Y still set via rebuildForTag_'s autoScaleY_(yInit)) and removed the per-tick broadcastTimeRange call from DashboardEngine.onLiveTick; (3) 8d41c48 flipped FastSenseWidget.LiveViewMode default 'reset'→'preserve' to stop applyViewMode('reset') from clobbering the user's zoom each tick, AND moved FastSenseToolbar.syncFollowState into a public methods block so FastSense.onXLimChanged's auto-disengage hook can actually reach it (ismethod was returning false for private methods, leaving Follow button stuck on after user-pan disengage). Live mode is now strictly "append data only"; axes change only on explicit user action (mouse, slider drag, Follow, Sync-All, broadcastTimeRangeNow). test_fastsense_follow_toggle 10/10, test_dashboard_time_sync_all_pages 5/5, test_dashboard_range_selector_integration 2/2.
...
| 260513-ovt | Preserve widget X and Y views across Live ticks — (1) added LiveViewMode='follow' guard inside FastSenseWidget.autoScaleY_, (2) removed `autoScaleY_(y)` from FastSenseWidget.refresh/update (initial Y still set via rebuildForTag_), (3) removed `broadcastTimeRange(tStart, tEnd)` from DashboardEngine.onLiveTick, (4) flipped FastSenseWidget.LiveViewMode default 'reset'→'preserve' (the actual per-widget X-reset culprit), (5) made FastSenseToolbar.syncFollowState public so FastSense.onXLimChanged's auto-disengage hook actually syncs the Follow button. Live mode is now strictly "append data only". test_fastsense_follow_toggle 10/10, test_dashboard_time_sync_all_pages 5/5, test_dashboard_range_selector_integration 2/2 | 2026-05-13 | 498a5f3, ca5be95, 8d41c48 | — | [260513-ovt-when-follow-button-is-pressed-y-axis-lim](./quick/260513-ovt-when-follow-button-is-pressed-y-axis-lim/) |
